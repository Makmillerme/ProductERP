...
export function DocumentFoldersEditor({
  ...
  const addFolder = useCallback(() => {
    ...
    const next = [...folders, { code, label: "Нова папка" }];
  ...
  return (
    <div className="grid gap-3">
      <div className="flex items-center justify-between gap-2">
        <Label className="text-sm font-medium">Папки документів</Label>
        <Button
          type="button"
          variant="outline"
          size="sm"
      ...
      {folders.length === 0 ? (
        <p className="text-sm text-muted-foreground py-3 rounded-md border border-dashed text-center">
          Немає папок. Натисніть «Додати папку», щоб створити.
        </p>
      ) : (

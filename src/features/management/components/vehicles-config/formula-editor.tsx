  ...
  return (
    <div className={cn("grid gap-2", className)}>
      <div className="flex items-center gap-2">
        <Label htmlFor={id}>Формула</Label>
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              aria-label="Доступні числові поля"
            >
          ...
          >
            ...
            <ul className="space-y-1 text-left">
              {numericFields.length === 0 ? (
                <li className="text-muted-foreground">Немає доступних полів</li>
              ) : (
              ...
              {numericFields.length > 6 && (
                <li className="text-muted-foreground text-[10px] pt-1">
                  +{numericFields.length - 6} інших — введіть {"{"} для пошуку
                </li>
              )}
            </ul>
          </TooltipContent>
        ...
        <Popover open={insertOpen} onOpenChange={setInsertOpen}>
          ...
          <PopoverContent align="end" className="w-64 p-4">
            <p className="mb-2 text-xs text-muted-foreground">
              Оберіть поле або введіть назву для пошуку
            </p>

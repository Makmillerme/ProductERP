...
import { parsePresetValues } from "@/features/vehicles/lib/field-utils";
import type { SelectOption } from "@/features/vehicles/lib/field-utils";
...
function validateOptionValue(value: string, dataType: DataType): string | null {
  const v = value.trim();
  if (!v) return "Значення не може бути порожнім";
  if (dataType === "string") return null;
  if (dataType === "integer") {
    const n = parseInt(v, 10);
    if (Number.isNaN(n) || String(n) !== v) return "Має бути ціле число (наприклад: 1, 2)";
    return null;
  }
  if (dataType === "float") {
    const n = parseFloat(v);
    if (Number.isNaN(n)) return "Має бути число (наприклад: 1.5, 2)";
    return null;
  }
  if (dataType === "boolean") {
    if (v !== "true" && v !== "false") return "Має бути true або false";
    return null;
  }
  return null;
}
...
export function OptionsEditor({
  ...
  const addOption = useCallback(() => {
    ...
    if (isValueOnly) {
      ...
      if (!v) {
        setError("Вкажіть значення опції");
        return;
      }
      const err = validateOptionValue(v, dataType);
      if (err) {
        setError(err);
        return;
      }
    ...
    if (used.has(valueStr)) {
      if (isValueOnly) {
        setError("Опція з таким значенням вже є");
        return;
      }
      let suffix = 0;
    ...
    setLabelInput("");
    setValueInput("");
  }, [labelInput, valueInput, options, onChange, dataType, isValueOnly]);

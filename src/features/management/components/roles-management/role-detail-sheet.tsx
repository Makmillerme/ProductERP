...
export function RoleDetailSheet({
  ...
  const handleSave = async () => {
    if (isCreate) {
      ...
      if (!trimmedName || !trimmedCode) {
        toast.error("Вкажіть назву та код ролі");
        return;
      }
      setSaving(true);
      try {
  ...
  const title = showLoading ? "Завантаження…" : isCreate ? "Нова роль" : (role?.name ?? "Права доступу");
      ...
      >
        ...
        <div className={SHEET_BODY_CLASS}>
          <div className={cn("flex min-w-0 flex-1 flex-col gap-4 p-2", SHEET_SCROLL_CLASS)}>
            {showLoading && (
              <p className="text-sm text-muted-foreground py-6 text-center">Завантаження даних ролі…</p>
            )}
            {isLoadFailed && (
            ...
            {!showLoading && (
              ...
              <div className="grid gap-2">
                <Label htmlFor="role-name">Назва</Label>
                <Input
                  id="role-name"
                  value={name}
              ...
              <div className="grid gap-2">
                ...
                />
                {isCreate && (
                  <p className="text-xs text-muted-foreground">Генерується автоматично з назви</p>
                )}
                {!isCreate && (
                  <p className="text-xs text-muted-foreground">Код ролі змінити не можна</p>
                )}
              </div>
              <div className="grid gap-2">
                <Label htmlFor="role-desc">Опис (необовʼязково)</Label>
                <Input
                  id="role-desc"
                  value={description}
            ...
            <h3 className="text-sm font-medium text-foreground pt-2">Права доступу</h3>
        ...
        <SheetFooter className={SHEET_FOOTER_CLASS}>
          <div className="flex w-full flex-wrap items-center justify-end gap-2">
            <Button
              ...
              disabled={saving}
            >
              {isSystemAdmin ? "Закрити" : "Скасувати"}
            </Button>
          ...
          {isSystemAdmin && (
            <p className="text-xs text-muted-foreground w-full text-center">
              Системна роль за замовчуванням. Має усі права.
            </p>
          )}
        </SheetFooter>
      </SheetContent>
